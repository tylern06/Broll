...
gulp.task("move-svgs", function() {
  gulp.src("./assets/images/**/*.svg").pipe(gulp.dest("./dist/images"));
  // .pipe(notify({
...
//   gulp.src("./assets/fonts/**/*{eot,svg,ttf,woff,woff2}").pipe(gulp.dest("./dist/fonts"));
//   // .pipe(notify({
...
gulp.task("lint", function() {
  return gulp.src(paths.scripts.src).pipe(jshint()).pipe(jshint.reporter("jshint-stylish"));
});

//browser-sync
gulp.task("browser-sync", function() {
  browserSync.init({
    proxy: paths.hostname,
    host: paths.hostname,
    open: "external"
  });
});
...
gulp.task("default", ["sass", "minify-scripts", "lint", "watch"]);
